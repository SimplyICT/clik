    ...
    <div>
      ...
      {detail && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
          onClick={() => { setDetail(null); setEditForm(null); }}>
          <div style={{ background: '#fff', borderRadius: 8, padding: 24, width: 520, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <h3 style={{ margin: 0 }}>{editForm ? 'Edit Request' : 'Request Details'}</h3>
              <button onClick={() => { setDetail(null); setEditForm(null); }}
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
                  <button onClick={() => setEditForm(null)}
                    style={{ padding: '8px 16px', borderRadius: 4, border: '1px solid #ddd', cursor: 'pointer', fontSize: 13 }}>Cancel</button>
                  <button onClick={handleSaveEdit}
                    style={{ padding: '8px 16px', borderRadius: 4, border: 'none', background: '#00d4ff', color: '#000', cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>Save Changes</button>
                </div>
              </div>
            ) : (
              <div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14, marginBottom: 16 }}>
                  ...
                  <div style={{ gridColumn: '1 / -1' }}><small style={{ color: '#888' }}>Description</small><div style={{ fontSize: 14, marginTop: 2 }}>{detail.description || '-'}</div></div>
                  ...
                  <div><small style={{ color: '#888' }}>Customer</small><div style={{ fontSize: 14, marginTop: 2 }}>{detail.customerName || customers.find(c => c.id === detail.customerId)?.name || '-'}</div></div>
                ...
                <button onClick={() => setEditForm({ ...detail })}
                  style={{ padding: '8px 20px', borderRadius: 4, border: 'none', background: '#00d4ff', color: '#000', cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>Edit</button>
              </div>
            )}
          </div>
        </div>
