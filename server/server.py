...
"""
import json, bcrypt, secrets, os, re, uuid, urllib.request, urllib.error
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _serve_file(self):
        ...
        if path.startswith('/api/supabase/'):
            return self._supabase_proxy('GET')
        if path.startswith('/api/'):
            return self.json(404, {"error": "Not found"})

        # Determine which build to use
        build_dir = PORTAL_BUILD if path.startswith('/portal') else ADMIN_BUILD
        clean_path = path.lstrip('/')
        if clean_path.startswith('portal/'):
            clean_path = clean_path[7:] or 'index.html'

        if not clean_path:
            clean_path = 'index.html'
        full = build_dir / clean_path
